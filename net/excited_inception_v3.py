...
class SELayer(nn.Module):
    def __init__(self, channel, reduction=16):
        ...
        self.fc = nn.Sequential(
                nn.Linear(channel, reduction), ##0
                nn.ReLU(inplace=True),
                nn.Linear(reduction, channel), ##2
                nn.Sigmoid()
        )
        self.init_parameters(channel, reduction)
    ...
    # https://discuss.pytorch.org/t/initalize-the-weights-of-nn-convtranspose2d/946
    def init_parameters(self, channel, reduction):
        self.fc._modules['0'].weight.data.normal_(0, 1)
        self.fc._modules['0'].bias.data.zero_()

        self.fc._modules['2'].weight.data.normal_(0, 1)
        self.fc._modules['2'].bias.data.zero_()
